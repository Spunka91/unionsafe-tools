function show(id, text) {
  ...
  copyButton.onclick = function () {
    ...
    setTimeout(function () { copyButton.textContent = 'Copy Text'; }, 1200);
  };
...
function generateGsapWording() {
  ...
  if (lower.includes('aircraft') || lower.includes('plane') || lower.includes('contact') || lower.includes('hit') || lower.includes('struck') || lower.includes('damage')) {
    ...
    actions.push('Document where contact occurred, what was visible to the crew, what signals or communication were used, and what prevented the movement from being stopped earlier.');
  }
  if (lower.includes('wet') || lower.includes('slip') || lower.includes('trip') || lower.includes('floor') || lower.includes('ice')) {
    factors.push('walking/working surface condition, warning signs, cones/barricades, lighting, drainage, footwear, and whether the area was corrected or isolated');
    actions.push('Identify when the condition was first observed, who was notified, whether the area was marked off, and what correction was needed.');
  }
  if (lower.includes('heat') || lower.includes('hot') || lower.includes('dehydration') || lower.includes('dizzy') || lower.includes('cramp')) {
    factors.push('heat exposure, workload, access to water/rest/shade/cooling, acclimatization, symptoms, and when supervision was notified');
    actions.push('Document temperature/conditions if known, task being performed, symptoms, cooling actions, hydration access, and who was notified.');
  }
  ...
  const text = 'Suggested GSAP / Safety Report Verbiage\n\n' +
    'Submitted concern:\n' + description + '\n\n' +
    'Direct safety focus based on what was typed:\n- ' + uniqueFactors.join('\n- ') + '\n\n' +
...
function generateInjuryWording() {
  ...
  if (lower.includes('heat') || lower.includes('hot') || lower.includes('dehydration') || lower.includes('dizzy') || lower.includes('cramp')) {
    details.push('Include the temperature/conditions if known, work location, workload, symptoms, access to water/rest/shade/cooling, and who was notified.');
  }
  if (lower.includes('slip') || lower.includes('trip') || lower.includes('fall') || lower.includes('wet') || lower.includes('ice')) {
    details.push('Include the walking surface condition, footwear, lighting, cones/warnings, photos, and whether the condition was reported or corrected.');
  }
  if (lower.includes('lift') || lower.includes('back') || lower.includes('bag') || lower.includes('shoulder') || lower.includes('knee')) {
    details.push('Include the task being performed, object weight/position if known, body movement, symptoms, and whether help or equipment was available.');
  }
  if (lower.includes('chemical') || lower.includes('odor') || lower.includes('glycol') || lower.includes('fuel') || lower.includes('dust')) {
    details.push('Include the exposure source, smell/visibility/air condition, duration, symptoms, SDS or air-quality concerns, and whether you were removed from exposure.');
  }
  if (details.length === 0) {
    details.push('Include the exact work task, location, time, symptoms, condition/exposure involved, who was notified, and what response occurred.');
  }
  ...
  const text = 'Suggested Injury Reporting Language\n\n' +
    ...
    'Based on what was typed, make sure your report includes:\n- ' + details.join('\n- ') + '\n\n' +
    ...
    '3. The work condition or exposure involved.\n' +
    ...
    'Reminder:\nReport facts clearly and promptly. Do not guess. Do not minimize symptoms. This tool does not guarantee workers’ compensation acceptance; it helps explain the work connection clearly.';
