...
function standardsFor(input) {
  ...
  if (t.includes('chemical') || t.includes('glycol') || t.includes('fuel') || t.includes('odor') || t.includes('smell') || t.includes('sds')) {
    items.push('Hazard Communication: 29 CFR 1910.1200. Review SDS access, labeling, chemical exposure communication, and training.');
  }
...
function contactBody() {
  ...
  const mail = document.getElementById('mailingList').checked ? 'Yes' : 'No';
...
function handleContactSubmit() {
  show('contactOutput', 'Thank you. Your information is being submitted. First-time setup note: FormSubmit may send an activation email to unionsafe141@gmail.com. Once that email is confirmed, future submissions will come through automatically.');
  setTimeout(() => {
